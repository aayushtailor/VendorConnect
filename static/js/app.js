...
const dummyVendors = [
  {
    id: 1,
    name: "Aayush Tailor",
    business_name: "Elegant Stitches",
    category: "Tailor",
    city: "Jaipur",
    description: "Custom clothing and modern designs.",
    phone: "9998887777",
  },
  {
    id: 2,
    name: "Rohan Jewels",
    business_name: "Rohan Goldsmith",
    category: "Jewellery",
    city: "Jaipur",
      ...
      <div class="p-5 text-gray-900">
        ...
        <div class="flex justify-between items-center">
          ...
          <div class="flex gap-2">
            ${
              v.phone
                ? `<a href="tel:${v.phone}" class="text-xs px-3 py-1 bg-indigo-600 text-white rounded-full hover:bg-indigo-700 transition">Call</a>`
                : ""
            }
            ${
              v.whatsapp
                ? `<a href="https://wa.me/${v.whatsapp}" target="_blank" class="text-xs px-3 py-1 bg-green-600 text-white rounded-full hover:bg-green-700 transition">WhatsApp</a>`
                : ""
            }
          </div>
        </div>
...
function render() {
  ...
  const filtered = dummyVendors.filter((v) => {
    const text =
      `${v.business_name} ${v.name} ${v.category} ${v.city}`.toLowerCase();
